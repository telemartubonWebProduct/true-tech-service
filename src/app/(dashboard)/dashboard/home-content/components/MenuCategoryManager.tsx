...
export default function MenuCategoryManager({ initialItems }: Props) {
  ...
  // Form state
  const [form, setForm] = useState({ iconUrl: "", alt: "", text: "", path: "", displayOrder: 0, isActive: true });
  ...
  const resetForm = () => {
    setForm({ iconUrl: "", alt: "", text: "", path: "", displayOrder: items.length, isActive: true });
    setEditingId(null);
    setImageFile(null);
    setShowForm(false);
  };
        ...
        <div className="p-5 bg-gray-900/80 border border-gray-700 rounded-xl space-y-4">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 content-start">
              <div className="md:col-span-2"><label className={labelClass}>ข้อความ (Text)</label><input className={inputClass} value={form.text} onChange={(e) => setForm({ ...form, text: e.target.value })} placeholder="แพ็กเกจมือถือ" /></div>
              <div><label className={labelClass}>ลิงก์ (Path)</label><input className={inputClass} value={form.path} onChange={(e) => setForm({ ...form, path: e.target.value })} placeholder="/topup" /></div>
