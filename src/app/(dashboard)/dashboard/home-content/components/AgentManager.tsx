...
export default function AgentManager({ initialAgents }: Props) {
  ...
  const [form, setForm] = useState({ name: "", phoneNumber: "", role: "", closedDeal: 0, photoUrl: "", displayOrder: 0, isActive: true });
  ...
  const resetForm = () => {
    setForm({ name: "", phoneNumber: "", role: "", closedDeal: 0, photoUrl: "", displayOrder: agents.length, isActive: true });
    setEditingId(null);
    setImageFile(null);
    setShowForm(false);
  };
        ...
        <div className="p-5 bg-gray-900/80 border border-gray-700 rounded-xl space-y-4">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 content-start">
              ...
              <div><label className={labelClass}>เบอร์โทร</label><input className={inputClass} value={form.phoneNumber} onChange={(e) => setForm({ ...form, phoneNumber: e.target.value })} placeholder="089-123-4567" /></div>
              <div className="md:col-span-2"><label className={labelClass}>ตำแหน่ง</label><input className={inputClass} value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} placeholder="เซลล์ประจำพื้นที่" /></div>
