...
export default function PackageForm({ pkg, onSuccess, onCancel }: PackageFormProps) {
  ...
  const [freebies, setFreebies] = useState<string[]>(() => {
    if (Array.isArray(pkg?.freebie)) return pkg.freebie;
    if (typeof pkg?.freebie === "string" && pkg.freebie) return pkg.freebie.split(",").map(f => f.trim());
    return [""];
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const body = {
        ...
        freebie: freebies.filter(f => f.trim() !== ""),
        speed,
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select
            ...
            className="w-full px-3 py-2 bg-gray-800/50 border border-gray-700 rounded-xl text-white text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
          >
            <option value="โปรเน็ตบ้าน">โปรเน็ตบ้าน</option>
            <option value="เน็ตซิมรายวัน">เน็ตซิมรายวัน</option>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={() => handleAddField(setFreebies)}
          ...
          <div className="space-y-2">
            {freebies.map((f, i) => (
              <div key={`freebie-${i}`} className="flex gap-2">
                <input
                  type="text"
                  value={f}
                  onChange={(e) => handleFieldChange(i, e.target.value, setFreebies)}
                ...
                <button
                  type="button"
                  onClick={() => handleRemoveField(i, setFreebies)}
                  className="p-2 text-gray-500 hover:text-red-400 transition-colors"
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                </button>
